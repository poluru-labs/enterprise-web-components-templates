...
export function renderSearch(route) {
  ...
  return `
    ${pageHeader({
      eyebrow: 'Find',
      title: 'Search',
      lead: query ? `Results for “${query}” across trips, travelers, approvals, expenses, and risk.` : 'Search trips, travelers, approvals, itineraries, expenses, and people.',
    })}
    <eds-card padded>
      <eds-search id="search-page-input" placeholder="Find a trip, traveler, or city" clearable value="${safeQuery}"></eds-search>
...
export function hydrateSearch(root, route) {
  ...
  const paint = (value) => {
    ...
    if (!hits.length) {
      results.innerHTML = '<eds-empty-state heading="No matches" description="Try a traveler, city, or trip code." icon="search"></eds-empty-state>';
      return;
    }
