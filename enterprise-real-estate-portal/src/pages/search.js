...
export function renderSearch(route) {
  ...
  return `
    ${pageHeader({
      ...
      title: 'Search',
      lead: query
        ? `Results for “${query}” across properties, leases, and work.`
        : 'Search properties, leases, and maintenance requests.',
    })}
    <eds-card padded>
      <eds-search id="search-page-input" placeholder="Search assets, leases, work…" clearable value="${safeQuery}"></eds-search>
...
export function hydrateSearch(root, route) {
  ...
  const paint = (value) => {
    ...
    if (!String(needle).trim()) {
      results.innerHTML = '<p class="muted mb-0">Type to search the Poluru Homes desk.</p>';
      return;
    }
    if (!hits.length) {
      results.innerHTML =
        '<eds-empty-state heading="No matches" description="Try a property name, tenant, or work order." icon="search"></eds-empty-state>';
      return;
    }
