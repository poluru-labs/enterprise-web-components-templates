...
export function propertyCard(item) {
  return `
    <content-card href="#/property/${item.id}">
      <div slot="header" class="section-title">
        <h2>${item.code}</h2>
      ...
      <div class="prop-meta">
        ...
        <span class="muted">${item.pm} · ${formatPercent(item.occupancy, 0)} · ${formatCurrency(item.noi)}</span>
      </div>
    </content-card>
  `;
}

export function propertyGrid(list) {
  return `
    <section class="card-grid" aria-label="Properties">
      ${list.map((item) => propertyCard(item)).join('')}
    </section>
  `;
}
  ...
  return `
    <content-card href="#/maintenance">
      ...
      <div class="wo-meta">
        ...
        <span class="muted">${item.property} · ${item.unit}</span>
        <span class="muted">${item.reporter} · ${item.date}</span>
      </div>
    </content-card>
  `;
}
