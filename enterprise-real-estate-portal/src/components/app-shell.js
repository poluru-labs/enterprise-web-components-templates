...
export class HavenShell extends HTMLElement {
  connectedCallback() {
  ...
  render() {
    this.innerHTML = `
      <div class="hv-shell">
        ...
        <div class="hv-main">
          ...
          >
            ...
            <eds-visually-hidden slot="search">Search assets</eds-visually-hidden>
            <eds-search slot="search" id="global-search" placeholder="Find a property, lease, or work order" clearable></eds-search>
            <eds-kbd slot="kbd" keys="⌘K"></eds-kbd>
            <eds-button slot="add" id="header-add" variant="primary" icon="plus">Log request</eds-button>
      ...
      <button class="hv-backdrop" id="hv-backdrop" type="button" aria-label="Hide sidebar" hidden></button>
      ...
      <eds-modal id="command-modal" heading="Jump to anything" close-on-backdrop close-on-escape>
        <eds-search id="command-search" placeholder="Properties, leases, work…" clearable></eds-search>
        <eds-list id="command-list" divided class="mt-3"></eds-list>
        <div slot="footer">
          <eds-button id="close-command" variant="tertiary">Close</eds-button>
        </div>
      </eds-modal>
      <eds-modal id="wo-modal" heading="Log a request" close-on-backdrop close-on-escape>
        <div class="stack">
          <eds-input id="wo-title" label="Issue" placeholder="AC short-cycling in 12B" icon="file"></eds-input>
          <eds-select id="wo-property" label="Property"></eds-select>
          <eds-select id="wo-reporter" label="From"></eds-select>
          <eds-select id="wo-type" label="Trade"></eds-select>
        </div>
        <div slot="footer" class="inline-actions">
          <eds-button id="save-wo" variant="primary">Log request</eds-button>
          <eds-button id="close-wo" variant="tertiary">Cancel</eds-button>
        </div>
      </eds-modal>
    `;

    const header = this.querySelector('haven-header');
    if (header) header.assetMix = assetMix;
  }
  ...
  hydrate() {
    ...
    const propertySelect = this.querySelector('#wo-property');
    if (propertySelect) {
      propertySelect.options = properties.map((item) => ({ label: item.name, value: item.name }));
      propertySelect.value = properties[0]?.name;
    }
    const reporterSelect = this.querySelector('#wo-reporter');
    if (reporterSelect) {
      reporterSelect.options = people.map((item) => ({ label: item.name, value: item.name }));
      reporterSelect.value = currentUser.name;
    }
    const typeSelect = this.querySelector('#wo-type');
    if (typeSelect) {
      typeSelect.options = tradeOptions;
      typeSelect.value = 'HVAC';
    }
    ...
    this.querySelector('#hv-backdrop')?.addEventListener('click', () => this.setSidebarOpen(false));
    ...
    this.querySelector('#command-list')?.addEventListener('eds-select', (event) => {
      ...
      if (item?.href === '#log-request') {
        this.querySelector('#wo-modal')?.show();
        return;
      }
    ...
    this.querySelector('#header-add')?.addEventListener('eds-click', () => this.querySelector('#wo-modal')?.show());
    ...
    this.querySelector('#profile-menu')?.addEventListener('eds-select', (event) => {
      ...
      if (value === 'signout') showToast({ message: 'Signed out of Poluru Homes', variant: 'warning' });
    });

    this.querySelector('#save-wo')?.addEventListener('eds-click', () => {
      const title = this.querySelector('#wo-title')?.value;
      if (!title?.trim()) {
        showToast({ message: 'Issue required', variant: 'warning' });
        return;
      }
      const record = addWorkOrder({
        title,
        property: this.querySelector('#wo-property')?.value,
        reporter: this.querySelector('#wo-reporter')?.value,
        type: this.querySelector('#wo-type')?.value,
      });
      this.querySelector('#wo-modal')?.close();
      showToast({ message: `${record.id.toUpperCase()} logged`, variant: 'success' });
      window.location.hash = '#/maintenance';
    });
    this.querySelector('#close-wo')?.addEventListener('eds-click', () => this.querySelector('#wo-modal')?.close());
  ...
  setSidebarOpen(open) {
    ...
    const backdrop = this.querySelector('#hv-backdrop');
  ...
  paintCrumbs(route) {
    ...
    if (!crumbs) return;
    crumbs.items = crumbItems(route, { properties, workspaceName });
  }
...
if (!customElements.get('haven-shell')) {
  customElements.define('haven-shell', HavenShell);
}
