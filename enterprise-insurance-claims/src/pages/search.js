...
export function renderSearch(route) {
  ...
  return `
    ${pageHeader({
      ...
      title: 'Search',
      lead: query
        ? `Results for “${query}” across claims, policies, and flags.`
        : 'Search claims, policies, and fraud flags.',
    })}
    <eds-card padded>
      <eds-search id="search-page-input" placeholder="Search claims, policies, flags…" clearable value="${safeQuery}"></eds-search>
...
export function hydrateSearch(root, route) {
  ...
  const paint = (value) => {
    ...
    if (!String(needle).trim()) {
      results.innerHTML = '<p class="muted mb-0">Type to search the Poluru Cover desk.</p>';
      return;
    }
    if (!hits.length) {
      results.innerHTML =
        '<eds-empty-state heading="No matches" description="Try a claim code, insured, or flag." icon="search"></eds-empty-state>';
      return;
    }
