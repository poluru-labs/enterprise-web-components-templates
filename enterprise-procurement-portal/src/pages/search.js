...
export function renderSearch(route) {
  ...
  return `
    ${pageHeader({
      ...
      title: 'Search',
      lead: query
        ? `Results for “${query}” across requests, suppliers, and contracts.`
        : 'Search purchase requests, suppliers, contracts, and people.',
    })}
    <eds-card padded>
      <eds-search id="search-page-input" placeholder="Search PRs, suppliers, contracts…" clearable value="${safeQuery}"></eds-search>
...
export function hydrateSearch(root, route) {
  ...
  const paint = (value) => {
    ...
    if (!String(needle).trim()) {
      results.innerHTML = '<p class="muted mb-0">Type to search the Poluru Works buy desk.</p>';
      return;
    }
    if (!hits.length) {
      results.innerHTML =
        '<eds-empty-state heading="No matches" description="Try a PR number, supplier, or owner." icon="search"></eds-empty-state>';
      return;
    }
