...
export function renderSearch(route) {
  ...
  return `
    ${pageHeader({
      ...
      title: 'Search',
      lead: query
        ? `Results for “${query}” across policies, controls, audits, and tasks.`
        : 'Search policies, controls, audits, tasks, and people.',
    })}
    <eds-card padded>
      <eds-search id="search-page-input" placeholder="Search policies, controls, audits…" clearable value="${safeQuery}"></eds-search>
...
export function hydrateSearch(root, route) {
  ...
  const paint = (value) => {
    ...
    if (!String(needle).trim()) {
      results.innerHTML = '<p class="muted mb-0">Type to search the Poluru Trust control room.</p>';
      return;
    }
    if (!hits.length) {
      results.innerHTML =
        '<eds-empty-state heading="No matches" description="Try a policy code, control, or owner." icon="search"></eds-empty-state>';
      return;
    }
