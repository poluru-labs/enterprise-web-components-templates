...
export class OrbitShell extends HTMLElement {
  connectedCallback() {
  ...
  render() {
    this.innerHTML = `
      <div class="orbit-shell">
        ...
        <div class="orbit-main">
          ...
          >
            ...
            <eds-visually-hidden slot="search">Search the fleet</eds-visually-hidden>
            <eds-search slot="search" id="global-search" placeholder="Find a unit, driver, or work order" clearable></eds-search>
            <eds-kbd slot="kbd" keys="⌘K"></eds-kbd>
            <eds-button slot="add" id="header-add" variant="primary" icon="plus">Assign vehicle</eds-button>
      ...
      <eds-modal id="command-modal" heading="Jump to anything" close-on-backdrop close-on-escape>
        <eds-search id="command-search" placeholder="Units, drivers, work orders…" clearable></eds-search>
        <eds-list id="command-list" divided class="mt-3"></eds-list>
        <div slot="footer">
          <eds-button id="close-command" variant="tertiary">Close</eds-button>
        </div>
      </eds-modal>
      <eds-modal id="assign-modal" heading="Assign a vehicle" close-on-backdrop close-on-escape>
        <div class="stack">
          <eds-select id="as-vehicle" label="Vehicle"></eds-select>
          <eds-select id="as-driver" label="Driver"></eds-select>
          <eds-select id="as-yard" label="Yard"></eds-select>
          <eds-select id="as-type" label="Type"></eds-select>
        </div>
        <div slot="footer" class="inline-actions">
          <eds-button id="save-assign" variant="primary">Assign</eds-button>
          <eds-button id="close-assign" variant="tertiary">Cancel</eds-button>
        </div>
      </eds-modal>
    `;

    const header = this.querySelector('orbit-header');
    if (header) header.dispatchStages = dispatchStages;
  }
  ...
  hydrate() {
    ...
    const vehicleSelect = this.querySelector('#as-vehicle');
    if (vehicleSelect) {
      vehicleSelect.options = vehicles.map((item) => ({ label: `${item.unit} · ${item.make}`, value: item.unit }));
      vehicleSelect.value = vehicles[2]?.unit;
    }
    const driverSelect = this.querySelector('#as-driver');
    if (driverSelect) {
      driverSelect.options = drivers.map((item) => ({ label: item.name, value: item.name }));
      driverSelect.value = 'Asha Poluru';
    }
    const yardSelect = this.querySelector('#as-yard');
    if (yardSelect) {
      yardSelect.options = [
        { label: 'Austin', value: 'Austin' },
        { label: 'Dallas', value: 'Dallas' },
      ];
      yardSelect.value = 'Austin';
    }
    const typeSelect = this.querySelector('#as-type');
    if (typeSelect) {
      typeSelect.options = typeOptions;
      typeSelect.value = 'Van';
    }
    ...
    this.querySelector('#orbit-backdrop')?.addEventListener('click', () => this.setSidebarOpen(false));
    ...
    this.querySelector('#command-list')?.addEventListener('eds-select', (event) => {
      ...
      if (item?.href === '#assign-vehicle') {
        this.querySelector('#assign-modal')?.show();
        return;
      }
    ...
    this.querySelector('#header-add')?.addEventListener('eds-click', () => this.querySelector('#assign-modal')?.show());
    ...
    this.querySelector('#profile-menu')?.addEventListener('eds-select', (event) => {
      ...
      if (value === 'signout') showToast({ message: 'Signed out of Poluru Yards', variant: 'warning' });
    });

    this.querySelector('#save-assign')?.addEventListener('eds-click', () => {
      const vehicle = this.querySelector('#as-vehicle')?.value;
      const driver = this.querySelector('#as-driver')?.value;
      if (!vehicle || !driver) {
        showToast({ message: 'Pick a vehicle and a driver', variant: 'warning' });
        return;
      }
      const record = addAssignment({
        vehicle,
        driver,
        yard: this.querySelector('#as-yard')?.value,
      });
      this.querySelector('#assign-modal')?.close();
      showToast({ message: `${record.unit} assigned to ${driver}`, variant: 'success' });
      window.location.hash = `#/vehicle/${record.id}`;
    });
    this.querySelector('#close-assign')?.addEventListener('eds-click', () => this.querySelector('#assign-modal')?.close());
  ...
  setSidebarOpen(open) {
    ...
    const backdrop = this.querySelector('#orbit-backdrop');
  ...
  paintCrumbs(route) {
    ...
    if (!crumbs) return;
    crumbs.items = crumbItems(route, { vehicles, workspaceName });
  }
...
if (!customElements.get('orbit-shell')) {
  customElements.define('orbit-shell', OrbitShell);
}
