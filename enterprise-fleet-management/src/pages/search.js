...
export function renderSearch(route) {
  ...
  return `
    ${pageHeader({
      ...
      title: 'Search',
      lead: query
        ? `Results for “${query}” across units, drivers, and work orders.`
        : 'Search vehicles, drivers, maintenance, and inspections.',
    })}
    <eds-card padded>
      <eds-search id="search-page-input" placeholder="Search units, drivers, work orders…" clearable value="${safeQuery}"></eds-search>
...
export function hydrateSearch(root, route) {
  ...
  const paint = (value) => {
    ...
    if (!String(needle).trim()) {
      results.innerHTML = '<p class="muted mb-0">Type to search the Poluru Yards fleet desk.</p>';
      return;
    }
    if (!hits.length) {
      results.innerHTML =
        '<eds-empty-state heading="No matches" description="Try a unit number, driver, or work order." icon="search"></eds-empty-state>';
      return;
    }
