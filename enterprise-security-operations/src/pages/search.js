...
export function renderSearch(route) {
  ...
  return `
    ${pageHeader({
      ...
      title: 'Search',
      lead: query
        ? `Results for “${query}” across alerts, incidents, CVEs, and playbooks.`
        : 'Search alerts, incidents, vulnerabilities, investigations, and people.',
    })}
    <eds-card padded>
      <eds-search id="search-page-input" placeholder="Search alerts, incidents, CVEs…" clearable value="${safeQuery}"></eds-search>
...
export function hydrateSearch(root, route) {
  ...
  const paint = (value) => {
    ...
    if (!String(needle).trim()) {
      results.innerHTML = '<p class="muted mb-0">Type to search the Poluru Shield desk.</p>';
      return;
    }
    if (!hits.length) {
      results.innerHTML =
        '<eds-empty-state heading="No matches" description="Try an incident code, CVE, or owner." icon="search"></eds-empty-state>';
      return;
    }
