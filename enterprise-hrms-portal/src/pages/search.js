...
export function renderSearch(route) {
  ...
  return `
    ${pageHeader({
      ...
      title: 'Search',
      lead: query
        ? `Results for “${query}” across people, leave, reqs, and courses.`
        : 'Search people, leave records, requisitions, and learning courses.',
    })}
    <eds-card padded>
      <eds-search id="search-page-input" placeholder="Search people, leave, reqs…" clearable value="${safeQuery}"></eds-search>
...
export function hydrateSearch(root, route) {
  ...
  const paint = (value) => {
    ...
    if (!String(needle).trim()) {
      results.innerHTML = '<p class="muted mb-0">Type to search the Poluru People catalog.</p>';
      return;
    }
    if (!hits.length) {
      results.innerHTML =
        '<eds-empty-state heading="No matches" description="Try a name, req title, or course." icon="search"></eds-empty-state>';
      return;
    }
