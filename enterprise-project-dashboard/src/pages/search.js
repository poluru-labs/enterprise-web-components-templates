...
export function renderSearch(route) {
  ...
  return `
    ${pageHeader({
      ...
      title: 'Search',
      lead: query
        ? `Results for “${query}” across projects, tasks, risks, reports, and people.`
        : 'Search projects, tasks, risks, reports, and people.',
    })}
    <eds-card padded>
      <eds-search id="search-page-input" placeholder="Find a project, task, or teammate" clearable value="${safeQuery}"></eds-search>
...
export function hydrateSearch(root, route) {
  ...
  const paint = (value) => {
    ...
    if (!hits.length) {
      results.innerHTML =
        '<eds-empty-state heading="No matches" description="Try a project name, task title, or Poluru teammate." icon="search"></eds-empty-state>';
      return;
    }
