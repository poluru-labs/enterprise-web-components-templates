...
export class KeystoneShell extends HTMLElement {
  connectedCallback() {
  ...
  render() {
    this.innerHTML = `
      <div class="ks-shell">
        ...
        <div class="ks-main">
          ...
          >
            ...
            <eds-visually-hidden slot="search">Search jobs</eds-visually-hidden>
            <eds-search slot="search" id="global-search" placeholder="Find a site, RFI, or trade" clearable></eds-search>
            <eds-kbd slot="kbd" keys="⌘K"></eds-kbd>
            <eds-button slot="add" id="header-add" variant="primary" icon="plus">Log RFI</eds-button>
      ...
      <button class="ks-backdrop" id="ks-backdrop" type="button" aria-label="Hide sidebar" hidden></button>
      ...
      <eds-modal id="command-modal" heading="Jump to anything" close-on-backdrop close-on-escape>
        <eds-search id="command-search" placeholder="Sites, RFIs, trades…" clearable></eds-search>
        <eds-list id="command-list" divided class="mt-3"></eds-list>
        <div slot="footer">
          <eds-button id="close-command" variant="tertiary">Close</eds-button>
        </div>
      </eds-modal>
      <eds-modal id="rfi-modal" heading="Log an RFI" close-on-backdrop close-on-escape>
        <div class="stack">
          <eds-input id="rfi-title" label="Question" placeholder="Rebar at grid C/4" icon="file"></eds-input>
          <eds-select id="rfi-site" label="Job site"></eds-select>
          <eds-select id="rfi-author" label="From"></eds-select>
          <eds-select id="rfi-trade" label="Trade"></eds-select>
          <eds-date-picker id="rfi-due" label="Need by"></eds-date-picker>
        </div>
        <div slot="footer" class="inline-actions">
          <eds-button id="save-rfi" variant="primary">Log RFI</eds-button>
          <eds-button id="close-rfi" variant="tertiary">Cancel</eds-button>
        </div>
      </eds-modal>
    `;

    const header = this.querySelector('keystone-header');
    if (header) header.buildStages = buildStages;
  }
  ...
  hydrate() {
    ...
    const siteSelect = this.querySelector('#rfi-site');
    if (siteSelect) {
      siteSelect.options = sites.map((item) => ({ label: item.name, value: item.name }));
      siteSelect.value = sites[0]?.name;
    }
    const authorSelect = this.querySelector('#rfi-author');
    if (authorSelect) {
      authorSelect.options = people.map((item) => ({ label: item.name, value: item.name }));
      authorSelect.value = currentUser.name;
    }
    const tradeSelect = this.querySelector('#rfi-trade');
    if (tradeSelect) {
      tradeSelect.options = tradeOptions;
      tradeSelect.value = 'Structural steel';
    }
    ...
    this.querySelector('#ks-backdrop')?.addEventListener('click', () => this.setSidebarOpen(false));
    ...
    this.querySelector('#command-list')?.addEventListener('eds-select', (event) => {
      ...
      if (item?.href === '#log-rfi') {
        this.querySelector('#rfi-modal')?.show();
        return;
      }
    ...
    this.querySelector('#header-add')?.addEventListener('eds-click', () => this.querySelector('#rfi-modal')?.show());
    ...
    this.querySelector('#profile-menu')?.addEventListener('eds-select', (event) => {
      ...
      if (value === 'signout') showToast({ message: 'Signed out of Poluru Builds', variant: 'warning' });
    });

    this.querySelector('#save-rfi')?.addEventListener('eds-click', () => {
      const title = this.querySelector('#rfi-title')?.value;
      if (!title?.trim()) {
        showToast({ message: 'Question required', variant: 'warning' });
        return;
      }
      const record = addRfi({
        title,
        site: this.querySelector('#rfi-site')?.value,
        author: this.querySelector('#rfi-author')?.value,
        due: this.querySelector('#rfi-due')?.value,
      });
      this.querySelector('#rfi-modal')?.close();
      showToast({ message: `${record.number} logged`, variant: 'success' });
      window.location.hash = '#/rfis';
    });
    this.querySelector('#close-rfi')?.addEventListener('eds-click', () => this.querySelector('#rfi-modal')?.close());
  ...
  setSidebarOpen(open) {
    ...
    const backdrop = this.querySelector('#ks-backdrop');
  ...
  paintCrumbs(route) {
    ...
    if (!crumbs) return;
    crumbs.items = crumbItems(route, { sites, workspaceName });
  }
...
if (!customElements.get('keystone-shell')) {
  customElements.define('keystone-shell', KeystoneShell);
}
