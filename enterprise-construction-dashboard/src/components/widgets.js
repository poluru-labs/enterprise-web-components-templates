...
export function siteCard(item) {
  return `
    <content-card href="#/site/${item.id}">
      <div slot="header" class="section-title">
        <h2>${item.code}</h2>
      ...
      <div class="site-meta">
        ...
        <span class="muted">${item.city} · ${statusLabel(item.phase)}</span>
        <span class="muted">${item.pm} · ${item.complete}% · ${formatCurrency(item.spent)}</span>
      </div>
    </content-card>
  `;
}

export function siteGrid(list) {
  return `
    <section class="card-grid" aria-label="Job sites">
      ${list.map((item) => siteCard(item)).join('')}
    </section>
  `;
}

export function rfiCard(item) {
  return `
    <content-card href="#/rfis">
      <div slot="header" class="section-title">
        <h2>${item.number}</h2>
        ${statusChip(item.status)}
      </div>
      <div class="rfi-meta">
        ...
        <span class="muted">${item.author} → ${item.to} · due ${item.due}</span>
      </div>
    </content-card>
  `;
}

export function rfiGrid(list) {
  return `
    <section class="card-grid" aria-label="RFIs">
      ${list.map((item) => rfiCard(item)).join('')}
    </section>
  `;
}

export function subCard(item) {
  return `
    <content-card href="#/subcontractors">
      <div slot="header" class="section-title">
        <h2>${item.name}</h2>
        ${statusChip(item.status)}
      </div>
      <div class="sub-meta">
        ...
        <span class="muted">${item.site} · ${item.contact}</span>
...
export function subGrid(list) {
  return `
    <section class="card-grid" aria-label="Subcontractors">
      ${list.map((item) => subCard(item)).join('')}
    </section>
  `;
}
