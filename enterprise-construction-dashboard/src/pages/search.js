...
export function renderSearch(route) {
  ...
  return `
    ${pageHeader({
      ...
      title: 'Search',
      lead: query
        ? `Results for “${query}” across jobs, RFIs, and trades.`
        : 'Search job sites, RFIs, subcontractors, and safety reports.',
    })}
    <eds-card padded>
      <eds-search id="search-page-input" placeholder="Search jobs, RFIs, trades…" clearable value="${safeQuery}"></eds-search>
...
export function hydrateSearch(root, route) {
  ...
  const paint = (value) => {
    ...
    if (!String(needle).trim()) {
      results.innerHTML = '<p class="muted mb-0">Type to search the Poluru Builds jobs desk.</p>';
      return;
    }
    if (!hits.length) {
      results.innerHTML =
        '<eds-empty-state heading="No matches" description="Try a job name, RFI number, or trade." icon="search"></eds-empty-state>';
      return;
    }
