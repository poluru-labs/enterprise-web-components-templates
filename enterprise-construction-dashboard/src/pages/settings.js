...
export function renderSettings() {
  return `
    ${pageHeader({
      eyebrow: 'Workspace',
      title: 'Settings',
      lead: 'Poluru Builds jobs-desk defaults, RFI SLAs, and Keystone notifications.',
    })}
    <div class="row g-3">
      <div class="col-lg-6">
        ${sheet({
          ...
          body: `
            ...
            <eds-number-input class="mt-3" id="set-rfi" label="RFI due (days)" value="7" min="1"></eds-number-input>
            <eds-switch class="mt-3" label="Safety digest to Nikhil Poluru" checked></eds-switch>
...
export function hydrateSettings(root) {
  ...
  root.querySelector('#save-settings')?.addEventListener('eds-click', () => {
    showToast({ message: 'Jobs settings saved', variant: 'success' });
  });
}
