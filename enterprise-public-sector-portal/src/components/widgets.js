...
export function requestGrid(cards) {
  return `
    <section class="card-grid" aria-label="Requests">
      ${cards
        .map(
          (card) => `
        <content-card href="#/request/${card.id}">
          <div slot="header" class="section-title">
            <h2>${card.title}</h2>
            ${statusChip(card.status)}
          </div>
          <p class="muted mb-1">${card.code} · ${card.ward}</p>
          <p class="muted mb-3">${card.department} · ${card.owner} · due ${card.due}</p>
          ${statusChip(card.sla)}
        </content-card>`,
        )
        .join('')}
...
export function sparkline(points, label) {
  ...
  const fillId = `civic-fill-${++sparkId}`;
  return `
...
export function slaMix(cards) {
  const groups = [
    { key: 'On track', tone: 'on-track', match: (item) => item.sla === 'On track' || item.sla === 'Met', items: [] },
    { key: 'Watch', tone: 'watch', match: (item) => item.sla === 'Watch', items: [] },
    { key: 'Overdue', tone: 'risk', match: (item) => item.sla === 'Overdue', items: [] },
  ];
  cards.forEach((card) => {
    const group = groups.find((entry) => entry.match(card)) ?? groups[1];
    group.items.push(card);
  });
  const total = Math.max(cards.length, 1);
  const open = cards.filter((card) => card.status !== 'Complete' && card.status !== 'Closed').length;

  return `
    <div class="health-mix">
      <div class="health-mix-hero">
        <strong>${open}</strong>
        <span>Open on this board · ${total} total</span>
      </div>
      ...
      <ul class="health-mix-legend">
        ${groups
          .map((group) => {
            ...
            const names = group.items.map((item) => item.title || item.code).join(', ') || 'None';
            return `
              <li>
