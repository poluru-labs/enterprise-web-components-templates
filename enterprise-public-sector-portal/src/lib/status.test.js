...
describe('status helpers', () => {
  it('maps civic statuses to design-system tones', () => {
    ...
    expect(statusTone('Open')).toBe('brand');
    expect(statusTone('In review')).toBe('brand');
  ...
  it('maps badge variants for request chips', () => {
    expect(badgeVariant('On track')).toBe('success');
    expect(badgeVariant('Open')).toBe('brand');
  });
});
