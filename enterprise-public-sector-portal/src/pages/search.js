...
export function renderSearch(route) {
  ...
  return `
    ${pageHeader({
      eyebrow: 'Find',
      title: 'Search',
      lead: query ? `Results for “${query}” across requests, permits, cases, departments, and people.` : 'Search requests, permits, cases, departments, budgets, and people.',
    })}
    <eds-card padded>
      <eds-search id="search-page-input" placeholder="Find a request, permit, or case" clearable value="${safeQuery}"></eds-search>
...
export function hydrateSearch(root, route) {
  ...
  const paint = (value) => {
    ...
    if (!hits.length) {
      results.innerHTML = '<eds-empty-state heading="No matches" description="Try a ward, permit code, or owner." icon="search"></eds-empty-state>';
      return;
    }
