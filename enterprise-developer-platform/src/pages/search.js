...
export function renderSearch(route) {
  ...
  return `
    ${pageHeader({
      eyebrow: 'Find',
      title: 'Search',
      lead: query ? `Results for “${query}” across services, deploys, environments, routes, and owners.` : 'Search services, deploys, environments, routes, and owners.',
    })}
    <eds-card padded>
      <eds-search id="search-page-input" placeholder="Find a service, deploy, or owner" clearable value="${safeQuery}"></eds-search>
...
export function hydrateSearch(root, route) {
  ...
  const paint = (value) => {
    ...
    if (!hits.length) {
      results.innerHTML = '<eds-empty-state heading="No matches" description="Try a service, route, or owner." icon="search"></eds-empty-state>';
      return;
    }
