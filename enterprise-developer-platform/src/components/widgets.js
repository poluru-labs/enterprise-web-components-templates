...
export function serviceGrid(cards) {
  return `
    <section class="card-grid" aria-label="Services">
      ${cards
        .map(
          (card) => `
        <content-card href="#/service/${card.id}">
          <div slot="header" class="section-title">
            <h2>${card.name}</h2>
            ${statusChip(card.status)}
          </div>
          <p class="muted mb-1">${card.code} · ${card.env} · ${card.version}</p>
          <p class="muted mb-3">${card.stack} · ${card.owner} · p95 ${card.p95}ms</p>
          <eds-progress-bar value="${Math.min(card.slo, 100)}" max="100" label="SLO" show-value></eds-progress-bar>
        </content-card>`,
        )
        .join('')}
    </section>
...
export function sparkline(points, label) {
  ...
  const fillId = `circuit-fill-${++sparkId}`;
  return `
...
export function healthMix(cards) {
  const groups = [
    ...
    { key: 'Failed', tone: 'risk', items: [] },
  ];
  cards.forEach((card) => {
    const status = card.status === 'Rolling' || card.status === 'Live' || card.status === 'Open' ? 'Healthy' : card.status;
    const group = groups.find((entry) => entry.key === status) ?? groups[1];
    group.items.push(card);
  });
  const total = Math.max(cards.length, 1);
  const healthy = groups[0].items.length;

  return `
    <div class="health-mix">
      <div class="health-mix-hero">
        <strong>${healthy}</strong>
        <span>Healthy now · ${total} on the board</span>
      </div>
