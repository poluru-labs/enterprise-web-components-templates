...
export function renderSearch(route) {
  ...
  return `
    ${pageHeader({
      eyebrow: 'Find',
      title: 'Search',
      lead: query ? `Results for “${query}” across events, guests, rooms, speakers, and sponsors.` : 'Search events, guests, rooms, speakers, sponsors, and reports.',
    })}
    <eds-card padded>
      <eds-search id="search-page-input" placeholder="Find an event, guest, or room" clearable value="${safeQuery}"></eds-search>
...
export function hydrateSearch(root, route) {
  ...
  const paint = (value) => {
    ...
    if (!hits.length) {
      results.innerHTML = '<eds-empty-state heading="No matches" description="Try an event, guest, or room." icon="search"></eds-empty-state>';
      return;
    }
