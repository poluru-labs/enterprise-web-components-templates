...
export function eventGrid(cards) {
  return `
    <section class="card-grid" aria-label="Events">
      ${cards
        .map(
          (card) => `
        <content-card href="#/event/${card.id}">
          <div slot="header" class="section-title">
            <h2>${card.name}</h2>
            ${statusChip(card.status)}
          </div>
          <p class="muted mb-1">${card.code} · ${card.venue}</p>
          <p class="muted mb-3">${card.start} – ${card.end} · ${formatNumber(card.registered)} of ${formatNumber(card.capacity)}</p>
          <eds-progress-bar value="${Math.round((card.registered / card.capacity) * 100)}" max="100" label="Registered" show-value></eds-progress-bar>
        </content-card>`,
        )
        .join('')}
    </section>
...
export function sparkline(points, label) {
  ...
  const fillId = `gather-fill-${++sparkId}`;
  return `
    ...
    <ol class="cadence-list">
      ${items
        .map((item) => {
          ...
          const current = item.status === 'Today' || item.status === 'Next' || item.status === 'Live';
          return `
...
export function houseMix(cards) {
  const groups = [
    { key: 'Live', tone: 'risk', items: [] },
    { key: 'Upcoming', tone: 'watch', items: [] },
    { key: 'Complete', tone: 'on-track', items: [] },
  ];
  cards.forEach((card) => {
    const status = card.status === 'Pending' || card.status === 'Draft' ? 'Upcoming' : card.status;
    const group = groups.find((entry) => entry.key === status) ?? groups[1];
  ...
  return `
    <div class="health-mix">
      <div class="health-mix-hero">
        <strong>${live}</strong>
        <span>Live now · ${total} on the book</span>
      </div>
