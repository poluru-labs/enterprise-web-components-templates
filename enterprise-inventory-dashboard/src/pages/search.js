...
export function renderSearch(route) {
  ...
  return `
    ${pageHeader({
      ...
      title: 'Search',
      lead: query
        ? `Results for “${query}” across SKUs, purchase orders, warehouses, alerts, suppliers, and team.`
        : 'Search SKUs, purchase orders, warehouses, alerts, suppliers, and team members.',
    })}
    <eds-card padded>
      <eds-search id="search-page-input" placeholder="Find a SKU, order, or warehouse" clearable value="${safeQuery}"></eds-search>
...
export function hydrateSearch(root, route) {
  ...
  const paint = (value) => {
    ...
    if (!String(needle).trim()) {
      results.innerHTML = '<p class="muted mb-0">Type to search the inventory catalog.</p>';
      return;
    }
    if (!hits.length) {
      results.innerHTML =
        '<eds-empty-state heading="No matches" description="Try a SKU, supplier name, warehouse, or alert title." icon="search"></eds-empty-state>';
      return;
    }
