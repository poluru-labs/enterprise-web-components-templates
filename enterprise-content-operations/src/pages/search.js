...
export function renderSearch(route) {
  ...
  return `
    ${pageHeader({
      ...
      title: 'Search',
      lead: query
        ? `Results for “${query}” across pieces, locales, and assets.`
        : 'Search pieces, locales, and assets.',
    })}
    <eds-card padded>
      <eds-search id="search-page-input" placeholder="Search pieces, locales, assets…" clearable value="${safeQuery}"></eds-search>
...
export function hydrateSearch(root, route) {
  ...
  const paint = (value) => {
    ...
    if (!String(needle).trim()) {
      results.innerHTML = '<p class="muted mb-0">Type to search the Fieldline Press desk.</p>';
      return;
    }
    if (!hits.length) {
      results.innerHTML =
        '<eds-empty-state heading="No matches" description="Try a piece code, locale, or asset." icon="search"></eds-empty-state>';
      return;
    }
