...
export class SignalShell extends HTMLElement {
      ...
      <div class="sig-shell">
        ...
        <div class="sig-main">
          <signal-header product="${productName}" workspace="${workspaceName}" period="${workspace.period}" inbox-count="${inboxItems.length}">
            ...
            <eds-visually-hidden slot="search">Search security operations</eds-visually-hidden>
            <eds-search slot="search" id="global-search" placeholder="Search incidents, assets, people" clearable></eds-search>
            ...
            <eds-button slot="alert" id="header-alert" variant="primary" icon="plus">New incident</eds-button>
  ...
  hydrate() {
    ...
    this.querySelector('#profile-menu')?.addEventListener('eds-select', (event) => {
      ...
      if (value === 'signout') showToast({ message: 'Signed out of Sentinel', variant: 'warning' });
    });
