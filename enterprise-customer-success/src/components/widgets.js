...
export function accountGrid(cards) {
  return `
    <section class="card-grid" aria-label="Accounts">
      ${cards
        .map(
          (card) => `
        <content-card href="#/account/${card.id}">
          <div slot="header" class="section-title">
            <h2>${card.name}</h2>
            ${statusChip(card.status)}
          </div>
          <p class="muted mb-2">${card.segment} · ${card.csm}</p>
          <p class="muted mb-3">${formatCurrency(card.arr)} ARR · renews ${card.renews}</p>
...
export function sparkline(points, label) {
  ...
  const fillId = `pulse-fill-${++sparkId}`;
  return `
...
export function healthMix(cards) {
  ...
  cards.forEach((card) => {
    const status = card.status === 'Onboarding' ? 'Healthy' : card.status;
  ...
  return `
    <div class="health-mix">
      <div class="health-mix-hero">
        <strong>${avg}</strong>
        <span>Portfolio health · ${total} accounts</span>
      </div>
