...
export function renderSearch(route) {
  ...
  return `
    ${pageHeader({
      eyebrow: 'Find',
      title: 'Search',
      lead: query ? `Results for “${query}” across accounts, renewals, onboarding, expansion, and people.` : 'Search accounts, renewals, onboarding, expansion, playbooks, and people.',
    })}
    <eds-card padded>
      <eds-search id="search-page-input" placeholder="Find an account, renewal, or CSM" clearable value="${safeQuery}"></eds-search>
...
export function hydrateSearch(root, route) {
  ...
  const paint = (value) => {
    ...
    if (!hits.length) {
      results.innerHTML = '<eds-empty-state heading="No matches" description="Try an account, CSM, or playbook name." icon="search"></eds-empty-state>';
      return;
    }
